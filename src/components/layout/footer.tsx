      ...
      <div className="mx-auto max-w-[1728px] px-6 pt-16 pb-10">
        <div className="grid grid-cols-2 gap-10 md:grid-cols-5">
          <div className="col-span-2 md:col-span-1">
            ...
            <p className="mt-4 text-[13px] font-normal leading-snug text-white/40">
              AI-powered legal receptionist.<br />24/7. English & Spanish.
            </p>
          </div>
          {Object.entries(cols).map(([title, links]) => (
            <div key={title}>
              <p className="text-[11px] font-normal uppercase tracking-wider text-white/50">{title}</p>
              <ul className="mt-4 space-y-2">
                {links.map((l) => (
                  <li key={l.href}>
                    <Link href={l.href} className="text-[13px] font-normal text-white/40 transition-colors hover:text-white/70">{l.name}</Link>
                  </li>
                ))}
              </ul>
            </div>
        ...
        <div className="mt-12 border-t border-white/15 pt-6 text-center">
          <p className="text-[12px] font-normal text-white/30">&copy; {new Date().getFullYear()} ClaireAI</p>
        </div>
      </div>
    </footer>
  );
